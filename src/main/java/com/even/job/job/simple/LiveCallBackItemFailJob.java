 ...
 */
public class LiveCallBackItemFailJob implements SimpleJob{
    @Log
    private Logger log;
    @Autowired
    ...
    public void execute(ShardingContext shardingContext) {
        ...
        Set<String> liveStateData = redisTemplate.opsForSet().members(Constant.Live_Call_Back_Start_Fail);
        mqSender.send("haixue_test3","LiveCallBackItemFailJob");
        liveStateData.forEach(data->{
            try {
                log.info("req the end to get the data");
                //TODO http
                if(true){
                    redisTemplate.opsForSet().remove(Constant.Live_Call_Back_Start_Fail,data);
                }
            }catch (Exception e){
